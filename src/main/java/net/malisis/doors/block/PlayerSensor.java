...
public class PlayerSensor extends Block {
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess world, int x, int y, int z) {
        ...
        if (dir == EAST) setBlockBounds(0, f, f, f, 2.0F * f, 1.0F - f);
        else if (dir == WEST) setBlockBounds(1 - f, f, f, 1, 2 * f, 1 - f);
        else if (dir == SOUTH) setBlockBounds(f, f, 0, 1 - f, 2 * f, f);
        else if (dir == NORTH) setBlockBounds(f, f, 1 - f, 1 - f, 2 * f, 1);
        else if (dir == DOWN) setBlockBounds(0.5F - f, 1 - f / 2, 0.5F - f, 0.5F + f, 1, 0.5F + f);
        else if (dir == UP) setBlockBounds(0.5F - f, 0, 0.5F - f, 0.5F + f, f / 2, 0.5F + f);
    }
    ...
    public void breakBlock(World world, int x, int y, int z, Block block, int metadata) {
        if (isPowered(metadata)) this.notifyPower(world, x, y, z);
